...
from google_find_my_ha.KeyBackup.cloud_key_decryptor import decrypt_eik, decrypt_aes_gcm
from google_find_my_ha.NovaApi.ExecuteAction.LocateTracker.decrypted_location import WrappedLocation
...
from NovaApi.util import flip_bits
from google_find_my_ha.SpotApi.GetEidInfoForE2eeDevices.get_eid_info_request import get_eid_info
from google_find_my_ha.SpotApi.GetEidInfoForE2eeDevices.get_owner_key import get_owner_key
...
def decrypt_location_response_locations(
        device_update_protobuf: DeviceUpdate_pb2.DeviceUpdate) -> LocationData | SemanticData:
    ...
    if loc.status == Common_pb2.Status.SEMANTIC:
        ...
        logger.debug(f"Time: {datetime.datetime.fromtimestamp(loc.time).strftime('%Y-%m-%d %H:%M:%S')}")
