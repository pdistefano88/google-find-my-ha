...
def decrypt_aes_gcm_with_derived_key(encrypted_data: bytes, private_key: bytes, key_type_string: bytes,
                                      derive_with_public_key: bool = False) -> bytes:
...
def derive_shared_secret(private_key_jwt: bytes, public_key: bytes) -> bytes:
    ...
    public_key_obj = ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), public_key)

    # Perform ECDH key exchange to calculate shared secret
    return private_key.exchange(ec.ECDH(), public_key_obj)


def decrypt_aes_gcm(key: bytes, encrypted_data_and_iv: bytes, additional_data: bytes | None = None,
...
def encrypt_aes_gcm(key: bytes, plaintext: bytes, additional_data: bytes | None = None,
                    iv_length: int = 12) -> bytes:
...
def decrypt_aes_cbc_no_padding(key: bytes, encrypted_data_and_iv: bytes, iv_length: int = 16) -> bytes:
    # IV is prepended to encrypted data
    iv = encrypted_data_and_iv[:iv_length]
    ciphertext = encrypted_data_and_iv[iv_length:]
