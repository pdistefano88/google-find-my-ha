...
from google_find_my_ha.proto_decoders import device_update_pb2, location_reports_upload_pb2
...
def parse_location_report_upload_protobuf(hex_string):
    location_reports = location_reports_upload_pb2.LocationReportsUpload()
...
def parse_device_update_protobuf(hex_string):
    device_update = device_update_pb2.DeviceUpdate()
    device_update.ParseFromString(bytes.fromhex(hex_string))
    return device_update


def parse_device_list_protobuf(hex_string: str) -> device_update_pb2.DevicesList:
    device_list = device_update_pb2.DevicesList()
...
def get_canonic_ids(device_list):
    ...
    for device in device_list.deviceMetadata:
        if device.identifierInformation.type == device_update_pb2.IDENTIFIER_ANDROID:
...
def print_location_report_upload_protobuf(hex_string):
    print(text_format.MessageToString(parse_location_report_upload_protobuf(hex_string),
                                      message_formatter=custom_message_formatter))


def print_device_update_protobuf(hex_string):
    print(text_format.MessageToString(parse_device_update_protobuf(hex_string),
                                      message_formatter=custom_message_formatter))


def print_device_list_protobuf(hex_string):
    print(
        text_format.MessageToString(parse_device_list_protobuf(hex_string), message_formatter=custom_message_formatter))
