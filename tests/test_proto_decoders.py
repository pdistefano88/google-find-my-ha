from google_find_my_ha.proto_decoders import (
    common_pb2,
    device_update_pb2,
    location_reports_upload_pb2,
    decoder,
)
from google_find_my_ha.proto_decoders.decoder import custom_message_formatter, get_canonic_ids


def test_parse_protobuf_roundtrips():
    update = device_update_pb2.DeviceUpdate()
    update.fcmMetadata.requestUuid = "req"
    assert decoder.parse_device_update_protobuf(update.SerializeToString().hex()) == update

    devices = device_update_pb2.DevicesList()
    ...
    upload = location_reports_upload_pb2.LocationReportsUpload(random1=1)
...
def test_get_canonic_ids_handles_android_and_non_android_devices():
    ...
    android.identifierInformation.type = device_update_pb2.IDENTIFIER_ANDROID
    ...
    tracker.identifierInformation.type = device_update_pb2.IDENTIFIER_SPOT
